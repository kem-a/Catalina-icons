...
fix_xml_errors() {
    local svg="$1"
    # Remove attributes with undeclared namespace prefixes (e.g. inkscape:, sodipodi:, sketch:)
    # These are editor leftovers where the namespace declaration was removed but attributes remained
    sed -i -E \
        -e 's/ (inkscape|sodipodi|sketch|xlink|osb):[a-zA-Z_-]+="[^"]*"//g' \
        -e 's/ (inkscape|sodipodi|sketch|xlink|osb):[a-zA-Z_-]+='"'"'[^'"'"']*'"'"'//g' \
        "$svg"
}
